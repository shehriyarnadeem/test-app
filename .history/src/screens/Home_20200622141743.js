...
const Home = () => {
  ...
  const Item = ({ title, image, checkbox, item, index }) => {
    return (
          ...
          {checkbox ? (
            <View style={{ display: "flex", justifyContent: "center" }}>
              <Checkbox
                ...
                onChange={() => handleProductSelection(item, index)}
              />
            </View>
          ) : null}
  ...
  const handleProductSelection = (item, index) => {
    let itemsArray = [];

    console.log(data[index]);
    if ((data[index].checked = !data[index].checked)) {
      console.log("sds");
    }
  };
