...
const Home = ({ navigation }) => {
  ...
  let searchMarginBottom = 110;
  let menuCardMaxHeight = "12%";
  const screenHeight = Dimensions.get("window").height;
  if (screenHeight > 700 && screenHeight < 800) {
    panelMarginTop = -180;
    headerImageHeight = "45%";
    panelMarginTopMinized = -173;
    searchMarginBottom = 110;
  } else if (screenHeight < 700) {
    headerImageHeight = "51%";
    panelMarginTop = -165;
    panelMarginTopMinized = -173;
    searchMarginBottom = 80;
    menuCardMaxHeight = "14%";
  } else if (screenHeight >= 800) {
    headerImageHeight = "50%";
    panelMarginTop = -190;
    panelMarginTopMinized = -205;
    searchMarginBottom = 80;
  }
          ...
          <>
            {minimizedHeader === false ? (
              <MenuCard
                ...
                activeScreen={screen}
                setTab={setTab}
                menuCardHeight={menuCardMaxHeight}
                setScreen={setScreen}
              />
            ) : (
              <MenuCardMinimized
    ...
    <>
      <View style={styles.container}>
        <ImageBackground
          style={{
            ...styles.headerImage,
            height: minimizedHeader === false ? headerImageHeight : "40%",
          }}
          resizeMode="stretch"
        ...
        >
          ...
          {minimizedHeader === false ? (
            <TextInput
              style={{ ...styles.searchBar, marginBottom: searchMarginBottom }}
              placeholder="Search or Filter"
            />
          ) : null}
          <Draggable
            imageSource={require("../../assets/icons/dragIcon.png")}
            renderSize={50}
            x={10}
            y={10}
